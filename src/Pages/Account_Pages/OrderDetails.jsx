...
// Match the API response shown in your Postman screenshot
function renderAddress(addr) {
  if (!addr) return "Address not available";
  const parts = [
    [addr.firstName, addr.lastName].filter(Boolean).join(" "),
    addr.address,                         // <- your API uses "address" (single string)
...
export default function OrderDetails() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const allOrders = Array.isArray(data) ? data : (data.items || data.data || []);
        setOrders(allOrders);
      } catch (e) {
  ...
  if (!orders || !orders.length) return <Container>No order details available.</Container>;
        ...
        <Row className="align-items-start g-4">
          ...
          <Col xs={12} md={9}>
            {orders.map((order, index) => {
              // Normalize items from API (price is commonly inside each item)
              ...
              const subtotal = products.reduce((sum, p) => sum + toNum(p.price) * toNum(p.quantity), 0);
              ...
              return (
                ...
                >
                  <h2 style={{ fontWeight: 700, color: "#002209", fontSize: 24 }}>Order #{index + 1}</h2>
